...
async function importExcelOldformat(filePath) {
  ...
  for (let index = 2; index < TOTAL; index++) {
    if (keys[index].indexOf("A") > -1) {
      let dataRollCall = {
        curriculumSectionId: workSheet[keys[index]].w,
        time: time,
        date: date,
        userId: workSheet[keys[index + 1]].w,
        note: workSheet[keys[index + 2]].w,
        status: workSheet[keys[index + 3]].w,
      };

      // Thêm dữ liệu public key và private key vào mảng fullData
      fullData.push({ dataRollCall });
