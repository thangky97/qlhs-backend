...
//     try {
//       let result = await QTrafficStatusImageResourceAccess.customInsert(req.payload);
//       if (result) {
//         resolve(result);
//       } else {
//         reject('failed');
...
async function get(req) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let result = await QTrafficStatusImageResourceAccess.customFind('findAll', filter, limit, skip);
      if (result) {
        let count = await QTrafficStatusImageResourceAccess.customFind('count', filter);
...
async function getHistoryInput(req) {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let skip = req.payload.skip;
      let limit = req.payload.limit;
      let result = await QTrafficStatusImageResourceAccess.FindHistoryInput('findAll', filter, limit, skip);
      if (result) {
        let count = await QTrafficStatusImageResourceAccess.FindHistoryInput('count', filter);
        resolve({ data: result, count: count });
      } else {
        resolve({ data: [], count: 0 });
      }
...
module.exports = {
  insert,
  get,
  robotInsertMedia,getHistoryInput
}