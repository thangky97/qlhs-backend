"use strict";
const Course_progressResourceAccess = require("../resourceAccess/Course_progressResourceAccess");
const Logger = require("../../../utils/logging");
const { STAFF_ROLE } = require("../Course_progressConstant");
const CommonFunctions = require("../../Common/CommonFunctions");
const DB = require("../../../models");
async function insert(req) {
  return new Promise(async (resolve, reject) => {
    try {
      let checkexistence = await DB.courseprogresses.findOne({
        where: {
      ...
      console.log("checkii", checkexistence);
      if (checkexistence) {
        ...
        let course_progressData = req.payload;
        //create new user
        let addResult = await Course_progressResourceAccess.insert(
          course_progressData
        );
...
async function find(req) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let course_videos = await Course_progressResourceAccess.customFind(
        filter,
        skip,
        limit,
        order
      ...
      if (course_videos && course_videos.length > 0) {
        let course_videosCount = await Course_progressResourceAccess.count(
          filter,
...
async function updateById(req) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let updateResult = await Course_progressResourceAccess.updateById(
        course_filestorsId,
        course_filesData
      );
      if (updateResult) {
